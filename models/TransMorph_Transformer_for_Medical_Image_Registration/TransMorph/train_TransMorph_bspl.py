...
def main():
    batch_size = 1
    train_dir = '/vol/miltank/users/fratzke/Documents/atlas-based-registration/data/train_pickle/'
    val_dir = '/vol/miltank/users/fratzke/Documents/atlas-based-registration/data/valid_pickle/'
    weights = [1, 0.02]
    ...
    train_set = datasets.AtlasPickle(sorted(glob.glob(train_dir + 'raw/*.pkl')),sorted(glob.glob(train_dir + 'seg/*.pkl')), transforms=None)
    val_set = datasets.AtlasPickle(sorted(glob.glob(val_dir + 'raw/*.pkl')),sorted(glob.glob(val_dir + 'seg/*.pkl')), transforms=None)
    ...
    for epoch in range(epoch_start, max_epoch):
        ...
        for data in train_loader:
            ...
            print("[main] x,y shape ", x.shape, y.shape, flush=True)
        ...
        with torch.no_grad():
            for data in val_loader:
                ...
                print("def_out shape:", def_out.shape)
                dsc = utils.dice_val(def_out.long(), y_seg.long(), 46)
                #dsc = utils.dice_val(def_out.long(), y_seg.long(), 150)
